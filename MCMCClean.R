...
###### this loads a data frame named with columns for
###### State, Year, Population, deaths for each drug type of interest
###### CMS region

y = cbind(data$CocaineOnlyDeathCount,data$CocaineSynthOpioidDeath,data$PsychostimulantsOnlyDeathCount,data$CocainePsychSynthOpioidDeath)
...
model_code=nimbleCode({
  ...
  for(t in 1:T){
    for(r in 1:num.reg){
      mu.p.reg[t,r,1:D] ~ dmnorm(mu.t[t,1:D],prec=Prec.p[1:D,1:D])
    }
  }
  for(t in 1:T){
    for(d in 1:D){
  ...
  Prec.p[1:D,1:D] ~ dwish(WisPsi[1:D,1:D],D+1)
...
mod_constants=list(T=T,P=P,S=S,D=D,num.reg = length(unique(data$CMS)),reg = data$CMS,adj=adj,num=num,WisPsi = diag(D))
...
mod_inits=list(J = jump.init,beta = matrix(.0001,S*T,D),p = array(.5,c(T,S,D)),mu.p.s = array(0,c(T,S,D)),mu.p.reg = array(0,c(T,length(unique(data$CMS)),D)),mu.t = matrix(0,T,D),Prec.p=diag(D))
...
mcmc_conf$addMonitors('theta','R','mu.p.s','mu.p.reg','Prec.p','p')
